...
{
  HINSTANCE hInstance = LoadLibraryA("miraconn.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load miraconn.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_RUSTDESK_CORE_MAIN rustdesk_core_main =
      (FUNC_RUSTDESK_CORE_MAIN)GetProcAddress(hInstance, "miraconn_core_main_args");
  if (!rustdesk_core_main)
  {
    std::cout << "Failed to get miraconn_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  FUNC_RUSTDESK_GET_APP_NAME get_miraconn_app_name = (FUNC_RUSTDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_miraconn_app_name");
  if (get_miraconn_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_miraconn_app_name(app_name_buffer, 512) == 0) {
