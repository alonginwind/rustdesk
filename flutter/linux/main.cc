...
#define RUSTDESK_LIB_PATH "libmiraconn.so"
...
bool flutter_rustdesk_core_main() {
   ...
   if (!librustdesk) {
      fprintf(stderr,"Failed to load \"libmiraconn.so\"\n");
      char* error;
   ...
   auto core_main = (RustDeskCoreMain) dlsym(librustdesk,"miraconn_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"miraconn_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
