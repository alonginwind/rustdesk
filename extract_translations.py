...
# The template lives outside web/ so that `flutter build web` does not copy the unminified
# source, comments and all, next to the obfuscated output it is generated from.
TEMPLATE = os.path.join(SCRIPT_DIR, 'flutter', 'load_bridge.tpl.js')
...
def unescape_rust_str(s):
    # The regex captures the raw Rust literal, so its escapes still carry the backslash;
    # json.dumps would double them and the browser would show a literal \" or \n.
    return re.sub(r'\\([nrt"\'\\])',
                  lambda m: ESC.get(m.group(1), m.group(1)), s)
...
for prefix, locale in sorted(FILE_TO_LOCALE.items()):
    ...
    with open(filepath, encoding='utf-8') as f:
        for line in f:
            ...
            if m:
                key, value = unescape_rust_str(m.group(1)), unescape_rust_str(m.group(2))
                if value:
