...
fn stop_rustdesk_servers() {
    let _ = run_cmds(&format!(
        ...
        crate::get_app_name_real().to_lowercase(),
    ));
}
...
mod desktop {
    ...
    impl Desktop {
        ...
        fn get_display_xauth_xwayland(&mut self) {
            let tray = format!("{} +--tray", crate::get_app_name_real().to_lowercase());
            for _ in 1..=10 {
...
fn switch_service(stop: bool) -> String {
    ...
    if !home.is_empty() && home != "/root" && !Config::get().is_empty() {
        let app_name_lower = crate::get_app_name_real().to_lowercase();
        let app_name0 = crate::get_app_name_real();
...
fn get_shortcuts_inhibitor_app_id() -> String {
    if is_flatpak() {
        ...
        format!("{}.desktop", crate::get_app_name_real().to_lowercase())
    }
}
